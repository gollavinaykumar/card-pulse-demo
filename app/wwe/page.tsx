...
// KPIs
const dashKpis = [
  { label: "CURRENT PRICE", value: "$13,050", sub: "PSA 9 · Last Sale", icon: DollarSign, color: RED, delta: "+10.2%" },
  { label: "30D VOLUME",    value: "58",      sub: "Sales (↑34%)",      icon: BarChart3,  color: RED_BRIGHT, delta: "+34%" },
  { label: "PRICE FLOOR",   value: "$12,500", sub: "3-day support",     icon: TrendingUp, color: "#ffffff",  delta: "+8.1%" },
  { label: "PSA 10 PREM",   value: "2.8×",    sub: "vs PSA 9 avg",     icon: Flame,      color: RED, delta: "+0.3×" },
];

// Grade breakdown
const gradeBreakdown = [
  { grade: "PSA 10", pct: 8,  count: 124, color: RED,        avgPrice: "$36,500" },
  { grade: "PSA 9",  pct: 35, count: 543, color: RED_BRIGHT, avgPrice: "$13,050" },
...
const PriceTooltip = ({ active, payload, label }: { active?: boolean; payload?: { value: number; name: string }[]; label?: string }) => {
  ...
  return (
    <div style={{ background: "#0c0c0c", border: "1px solid rgba(220,38,38,0.3)", borderRadius: 12, padding: "10px 16px", color: WHITE, fontSize: "0.8rem", boxShadow: "0 8px 40px rgba(0,0,0,0.8)" }}>
      ...
      {payload.map((p, i) => (
        <p key={i} style={{ fontWeight: 700, color: p.name === "price" ? RED : "#6b7280", marginBottom: 1 }}>
...
const ScatterTooltip = ({ active, payload }: { active?: boolean; payload?: { payload: { price: number; daysAgo: number; volume: number; label: string } }[] }) => {
  ...
  return (
    <div style={{ background: "#0c0c0c", border: "1px solid rgba(220,38,38,0.3)", borderRadius: 12, padding: "10px 16px", color: WHITE, fontSize: "0.8rem", boxShadow: "0 8px 40px rgba(0,0,0,0.8)" }}>
      <p style={{ fontWeight: 800, color: RED, marginBottom: 2, fontFamily: "Oswald, sans-serif" }}>{d.label}</p>
    ...
    <div style={{ minHeight: "100vh", background: "#050505", paddingBottom: 60 }}>
      ...
      <nav style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <span style={{ background: "rgba(220,38,38,0.12)", border: "1px solid rgba(220,38,38,0.3)", borderRadius: 5, padding: "2px 8px", fontSize: "0.55rem", fontWeight: 700, letterSpacing: "0.14em", textTransform: "uppercase", color: RED, fontFamily: "Oswald, sans-serif" }}>LIVE</span>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          <button onClick={() => router.push(`/wwe/players/${p.slug}`)} style={{
            background: "rgba(220,38,38,0.1)", border: "1px solid rgba(220,38,38,0.3)",
            borderRadius: 8, padding: "6px 16px", fontSize: "0.72rem", fontWeight: 700,
            color: RED, cursor: "pointer", fontFamily: "Oswald, sans-serif", textTransform: "uppercase", letterSpacing: "0.06em",
            transition: "all 0.2s",
      ...
      <div style={{ maxWidth: 1300, margin: "0 auto", padding: "28px 20px", display: "flex", flexDirection: "column", gap: 22 }}>
        ...
        <div className="animate-fade-up arena-card arena-card-glow" style={{
        ...
        }} onClick={() => router.push(`/wwe/players/${p.slug}`)}>
          {/* Spotlight sweep */}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12, zIndex: 1 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <div>
                <div className="arena-headline" style={{ fontSize: "3rem", color: RED, lineHeight: 1 }}>{p.priceChange}</div>
            ...
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
              {[
                ...
                <div key={s.l} style={{ background: "rgba(220,38,38,0.05)", border: "1px solid rgba(220,38,38,0.12)", borderRadius: 10, padding: "6px 12px" }}>
                  <div style={{ fontSize: "0.52rem", color: RED, textTransform: "uppercase", letterSpacing: "0.12em", fontWeight: 700, fontFamily: "Oswald, sans-serif" }}>{s.l}</div>
            ...
            {/* AI insight */}
            <div style={{ display: "flex", alignItems: "center", gap: 8, background: "rgba(220,38,38,0.06)", border: "1px solid rgba(220,38,38,0.15)", borderRadius: 9, padding: "7px 12px" }}>
              <Zap size={13} color={RED} />
              <span style={{ fontSize: "0.72rem", color: "#777" }}>
                <strong style={{ color: RED }}>AI SIGNAL:</strong> Phase 3 acceleration — 15-20% upside projected. Card Grade: S+
              </span>
            </div>
          </div>
        </div>
              ...
              <div key={kpi.label} className={`animate-fade-up-delay-${i + 1} arena-card arena-card-neon-edge`} style={{
                ...
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 3 }}>
                    <ArrowUpRight size={12} color={RED} />
                    <span style={{ fontSize: "0.68rem", fontWeight: 700, color: RED }}>{kpi.delta}</span>
                  </div>
                </div>
                <div style={{ fontSize: "1.5rem", fontWeight: 900, color: WHITE, lineHeight: 1, fontFamily: "Oswald, sans-serif" }}>{kpi.value}</div>
                <div className="arena-subhead" style={{ fontSize: "0.62rem", color: RED, marginTop: 4 }}>{kpi.label}</div>
            ...
            <div className="animate-fade-up-delay-2 arena-card" style={{ padding: 24 }}>
                ...
                <div style={{ display: "flex", gap: 5 }}>
                  {(["7d", "30d", "90d"] as const).map(r => (
                    <button key={r} onClick={() => setPriceRange(r)} style={{
                      ...
                      border: priceRange === r ? `1px solid ${RED}` : "1px solid rgba(255,255,255,0.06)",
                      background: priceRange === r ? "rgba(220,38,38,0.15)" : "transparent",
                      color: priceRange === r ? RED : "#555",
              ...
              <ResponsiveContainer width="100%" height={250}>
                <AreaChart data={displayPrices} margin={{ top: 4, right: 8, left: -5, bottom: 0 }}>
                  ...
                  <Area type="monotone" dataKey="price" stroke={RED} strokeWidth={2.5} fill="url(#priceGrad)" name="price"
                    dot={{ fill: RED, strokeWidth: 0, r: 3 }}
                    activeDot={{ fill: RED_BRIGHT, strokeWidth: 2, stroke: RED, r: 5 }}
              ...
              <ResponsiveContainer width="100%" height={200}>
                <ComposedChart data={displayPrices} margin={{ top: 4, right: 8, left: -5, bottom: 0 }}>
                  ...
                  <Bar yAxisId="vol" dataKey="volume" name="volume" radius={[3, 3, 0, 0]}>
                    {displayPrices.map((_, i) => <Cell key={i} fill={`rgba(220,38,38,${0.2 + (i / displayPrices.length) * 0.5})`} />)}
                  </Bar>
            ...
            <div className="animate-fade-up-delay-4 arena-card" style={{ padding: 24 }}>
              ...
              <ResponsiveContainer width="100%" height={240}>
                <ScatterChart margin={{ top: 10, right: 20, left: 0, bottom: 10 }}>
                  ...
                  <Scatter data={salesScatter.filter(d => d.label === "PSA 10")} fill={RED} opacity={0.95} name="PSA 10" />
              ...
              <div style={{ display: "flex", justifyContent: "center", gap: 20, marginTop: 10 }}>
                {[{ label: "PSA 10", color: RED }, { label: "PSA 9", color: "#fff" }, { label: "PSA 8", color: "#6b7280" }].map(g => (
          ...
          <div className="animate-fade-up-delay-2" style={{ display: "flex", flexDirection: "column", gap: 16, position: "sticky", top: 72 }}>
                ...
                {eventsTimeline.map((ev, i) => (
                  <div key={ev.id} style={{ display: "flex", gap: 12, position: "relative" }}>
                    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", flexShrink: 0, width: 20 }}>
                      <div style={{ width: 10, height: 10, borderRadius: 99, background: ev.type === "signal" || ev.type === "price" ? RED : "#555", boxShadow: ev.type === "signal" || ev.type === "price" ? `0 0 8px ${RED}88` : "none", zIndex: 1, flexShrink: 0 }} />
                      {i < eventsTimeline.length - 1 && <div style={{ width: 1.5, flex: 1, background: "rgba(220,38,38,0.1)", marginTop: 2 }} />}
                    </div>
                    <div style={{ paddingBottom: 18, flex: 1, minWidth: 0 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 3, flexWrap: "wrap" }}>
                        <span style={{ fontSize: "0.55rem", fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", color: ev.type === "signal" || ev.type === "price" ? RED : "#666", fontFamily: "Oswald, sans-serif" }}>{newsTypeLabel(ev.type)}</span>
                        ...
                        {ev.tag && (
                          <span style={{
                            ...
                            background: "rgba(220,38,38,0.12)", color: RED,
            ...
            <div className="arena-card" style={{ padding: 24 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {allPlayers.filter(pd => pd.player.slug !== "mahomes").map(pd => (
                  <div key={pd.player.slug} onClick={() => router.push(`/wwe/players/${pd.player.slug}`)} style={{
                    display: "flex", alignItems: "center", gap: 10,
                    background: "rgba(220,38,38,0.03)", borderRadius: 10, padding: "8px 10px",
                    ...
                    onMouseLeave={e => { (e.currentTarget as HTMLDivElement).style.borderColor = "rgba(255,255,255,0.04)"; (e.currentTarget as HTMLDivElement).style.background = "rgba(220,38,38,0.03)"; }}
                  >
                    <div style={{ width: 34, height: 34, borderRadius: 8, flexShrink: 0, background: `rgba(220,38,38,0.15)`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: "0.6rem", fontWeight: 900, color: WHITE, border: "1px solid rgba(220,38,38,0.2)", fontFamily: "Oswald, sans-serif" }}>{pd.player.initials}</div>
                    ...
                    <div style={{ textAlign: "right", flexShrink: 0 }}>
                      <div style={{ fontSize: "0.72rem", fontWeight: 700, color: pd.player.priceChange.startsWith("+") ? RED : "#6b7280" }}>{pd.player.priceChange}</div>
            ...
            <div className="arena-card" style={{ padding: "16px 20px" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                {[
                  { label: "ON FIRE", count: allPlayers.filter(pp => pp.player.status === "fire").length, color: RED_BRIGHT },
                  { label: "HEATING", count: allPlayers.filter(pp => pp.player.status === "heating").length, color: RED },
                  ...
                  <div key={stat.label} style={{ background: "rgba(220,38,38,0.04)", border: "1px solid rgba(220,38,38,0.08)", borderRadius: 9, padding: "9px 10px", textAlign: "center" }}>
