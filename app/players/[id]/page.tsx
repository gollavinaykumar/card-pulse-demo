...
const chartColors: Record<MetricKey, string> = { yards: RED, td: RED_B, rating: WHITE, rushYards: GREY };
...
const ChartTooltip = ({ active, payload, label }: { active?: boolean; payload?: { name: string; value: number; color: string }[]; label?: string }) => {
  ...
  return (
    <div style={{ background: DARK, border: `1px solid rgba(220,38,38,0.3)`, borderRadius: 12, padding: "10px 16px", color: WHITE, fontSize: "0.82rem", boxShadow: "0 8px 40px rgba(0,0,0,0.8)" }}>
...
const PieTooltip = ({ active, payload }: { active?: boolean; payload?: { name: string; value: number }[] }) => {
  if (!active || !payload?.length) return null;
  return (<div style={{ background: DARK, border: `1px solid rgba(220,38,38,0.3)`, borderRadius: 12, padding: "8px 14px", color: WHITE, fontSize: "0.82rem" }}><p style={{ fontWeight: 700 }}>{payload[0].name}: {payload[0].value}</p></div>);
};
    ...
    <div style={{ position: "relative", width: 136, height: 136 }}>
      <svg width="136" height="136" style={{ transform: "rotate(-90deg)" }}>
        ...
        <circle cx="68" cy="68" r={r} fill="none" stroke="url(#rGrad)" strokeWidth="9" strokeLinecap="round"
          ...
          style={{ transition: "stroke-dashoffset 1.5s cubic-bezier(0.65,0,0.35,1) 0.5s", filter: "drop-shadow(0 0 10px rgba(220,38,38,0.7))" }} />
        <defs><linearGradient id="rGrad" x1="0%" y1="0%" x2="100%" y2="0%"><stop offset="0%" stopColor={RED} /><stop offset="100%" stopColor={RED_B} /></linearGradient></defs>
      </svg>
      <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
        <span style={{ fontSize: "2rem", fontWeight: 900, color: RED, lineHeight: 1, fontFamily: "Oswald, sans-serif" }}>{score}</span>
  ...
  if (realImage) {
    return (
      <img src={realImage} alt={`${initials} card slab`} width={180} height={260}
        style={{ borderRadius: 10, filter: `drop-shadow(0 0 28px rgba(220,38,38,0.6))`, objectFit: "cover" }} />
    );
  }

  return (
    <svg width="180" height="252" viewBox="0 0 180 252" style={{ filter: `drop-shadow(0 0 28px ${gc})`, borderRadius: 12 }}>
      <rect x="3" y="3" width="174" height="246" rx="10" fill="#0a0a0a" stroke="rgba(220,38,38,0.2)" strokeWidth="2" />
      <rect x="3" y="3" width="174" height="26" rx="10" fill="#0c0c0c" /><rect x="3" y="20" width="174" height="9" fill="#0c0c0c" />
      <text x="90" y="20" textAnchor="middle" fill={RED} fontSize="9" fontWeight="800" fontFamily="Oswald, sans-serif" letterSpacing="0.12em">PANINI PRIZM</text>
      ...
      <rect x="12" y="192" width="156" height="34" rx="5" fill="#050505" />
      ...
      <text x="90" y="220" textAnchor="middle" fill={RED} fontSize="16" fontWeight="900" fontFamily="Oswald, sans-serif">PSA 9</text>
...
export default function PlayerPage() {
  ...
  if (!data) {
    return (
      <div style={{ minHeight: "100vh", background: "#050505", display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 16 }}>
        ...
        <button onClick={() => router.push("/")} style={{ background: "rgba(220,38,38,0.12)", border: `1px solid rgba(220,38,38,0.3)`, color: RED, padding: "8px 18px", borderRadius: 8, fontWeight: 700, fontSize: "0.82rem", cursor: "pointer", fontFamily: "Oswald, sans-serif", textTransform: "uppercase" }}>← BACK HOME</button>
      </div>
    );
  }
  ...
  const kpis = [
    { label: "CAREER YARDS",  value: cs.passing_yards.toLocaleString(), icon: TrendingUp, color: RED },
    { label: "TOUCHDOWNS",    value: String(cs.passing_touchdowns),     icon: Zap,         color: RED_B },
    { label: "CAREER WINS",   value: String(cs.wins),                   icon: Shield,      color: WHITE },
    { label: "SUPER BOWLS",   value: String(cs.super_bowls_won),        icon: Trophy,      color: RED },
    { label: "PASSER RATING", value: String(cs.passer_rating),          icon: Target,      color: GREY },
    { label: "CLUTCH SCORE",  value: `${am.clutch_rating}/100`,         icon: Flame,       color: RED_B },
  ];

  const metricBars = [
    { label: "Clutch Rating",    value: am.clutch_rating,                      max: 100, color: RED },
    { label: "Completion %",     value: Math.round(cs.completion_percentage),   max: 100, color: RED_B },
    { label: "Passer Rating",    value: Math.round(cs.passer_rating),          max: 158, color: WHITE },
    { label: "TD/INT Ratio",     value: Math.round(am.td_int_ratio * 20),      max: 100, color: RED },
    { label: "Yards / Attempt",  value: Math.round(am.yards_per_attempt * 10), max: 100, color: GREY },
    { label: "Playoff Win Rate", value: Math.round(am.playoff_win_rate * 100), max: 100, color: RED_B },
  ];

  const C: React.CSSProperties = {
    background: "linear-gradient(145deg, rgba(14,14,14,0.95), rgba(8,8,8,0.98))",
    border: "1px solid rgba(220,38,38,0.1)", borderRadius: 16, padding: 24,
    boxShadow: "0 4px 30px rgba(0,0,0,0.5), inset 0 1px 0 rgba(255,255,255,0.03)",
  };

  return (
    <div style={{ minHeight: "100vh", background: "#050505", paddingBottom: 60 }}>
      {/* ── Navbar ── */}
      <nav style={{ position: "sticky", top: 0, zIndex: 50, backdropFilter: "blur(28px)", WebkitBackdropFilter: "blur(28px)", background: "rgba(5,5,5,0.92)", borderBottom: "1px solid rgba(220,38,38,0.1)", padding: "0 28px", display: "flex", alignItems: "center", justifyContent: "space-between", height: 56 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <button onClick={() => router.push("/")} style={{ background: "rgba(220,38,38,0.08)", border: "1px solid rgba(220,38,38,0.2)", borderRadius: 8, width: 34, height: 34, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
            <ArrowLeft size={16} color={RED} />
          </button>
          <div style={{ width: 32, height: 32, borderRadius: 8, background: "linear-gradient(135deg, #dc2626, #b91c1c)", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 0 18px rgba(220,38,38,0.5)" }}>
            <Activity size={16} color="white" strokeWidth={2.5} />
          </div>
          <span style={{ fontFamily: "Oswald, sans-serif", fontWeight: 700, fontSize: "1.1rem", color: WHITE, textTransform: "uppercase", letterSpacing: "0.04em" }}>Card<span style={{ color: RED }}>Pulse</span></span>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
          <div className="live-dot" />
          <span style={{ fontSize: "0.65rem", color: RED, fontWeight: 700, fontFamily: "Oswald, sans-serif", letterSpacing: "0.08em" }}>LIVE</span>
        </div>
      </nav>
      ...
      <div style={{ maxWidth: 1300, margin: "0 auto", padding: "28px 20px", display: "flex", flexDirection: "column", gap: 24 }}>
        ...
        <div className="animate-fade-up arena-card arena-card-glow" style={{ display: "grid", gridTemplateColumns: "auto 1fr", gap: 28, padding: 28, position: "relative", overflow: "hidden" }}>
          ...
          <div style={{ position: "absolute", right: -80, top: -80, width: 350, height: 350, background: "radial-gradient(circle, rgba(220,38,38,0.08) 0%, transparent 65%)", pointerEvents: "none" }} />
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14, zIndex: 1 }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <div>
                <div className="arena-headline" style={{ fontSize: "2.4rem", color: p.priceChange.startsWith("+") ? RED : GREY, lineHeight: 1 }}>{p.priceChange}</div>
            ...
            <div style={{ display: "flex", gap: 7, flexWrap: "wrap" }}>
              <span style={{ background: "rgba(220,38,38,0.1)", border: "1px solid rgba(220,38,38,0.3)", borderRadius: 5, padding: "3px 11px", fontSize: "0.6rem", fontWeight: 800, letterSpacing: "0.12em", textTransform: "uppercase", color: RED, fontFamily: "Oswald, sans-serif" }}>{p.position}</span>
              ...
              {cs.super_bowls_won > 0 && <span style={{ background: "rgba(220,38,38,0.08)", border: "1px solid rgba(220,38,38,0.2)", borderRadius: 5, padding: "3px 11px", fontSize: "0.6rem", fontWeight: 800, letterSpacing: "0.12em", textTransform: "uppercase", color: RED_B, fontFamily: "Oswald, sans-serif" }}>{cs.super_bowls_won}× SB CHAMPION</span>}
            </div>
            ...
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
              {[
                ...
                <div key={s.label} style={{ background: "rgba(220,38,38,0.05)", border: "1px solid rgba(220,38,38,0.12)", borderRadius: 10, padding: "7px 13px" }}>
                  <div style={{ fontSize: "0.52rem", color: RED, textTransform: "uppercase", letterSpacing: "0.12em", fontWeight: 700, fontFamily: "Oswald, sans-serif" }}>{s.label}</div>
                  <div style={{ fontSize: "1.05rem", fontWeight: 800, color: WHITE, marginTop: 2 }}>{s.value}</div>
                </div>
              ))}
            </div>

            <div style={{ display: "flex", alignItems: "center", gap: 8, background: "rgba(220,38,38,0.06)", border: "1px solid rgba(220,38,38,0.15)", borderRadius: 9, padding: "9px 13px" }}>
              <Zap size={13} color={RED} />
              <span style={{ fontSize: "0.74rem", color: "#666" }}>
                <strong style={{ color: RED }}>AI SIGNAL:</strong>{" "}
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
              ...
              <div>
                <div style={{ display: "flex", gap: 4, marginBottom: 4 }}>
                  {[...Array(5)].map((_, i) => <Star key={i} size={12} fill={i < Math.round(p.overallScore / 20) ? RED : "#1a1a1a"} color={i < Math.round(p.overallScore / 20) ? RED : "#1a1a1a"} />)}
                </div>
          ...
          {kpis.map((kpi, i) => {
            ...
            return (
              <div key={kpi.label} className={`animate-fade-up-delay-${i + 1} arena-card arena-card-neon-edge`} style={{ padding: 18, cursor: "default" }}>
                ...
                <div className="arena-subhead" style={{ fontSize: "0.62rem", color: RED, marginTop: 5 }}>{kpi.label}</div>
              </div>
            );
          })}
        </div>
        ...
        <div className="animate-fade-up-delay-2 arena-card" style={{ padding: 24 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {data.news_feed.map(item => (
              <div key={item.id} style={{ display: "flex", gap: 14, alignItems: "flex-start", padding: "14px 16px", background: "rgba(220,38,38,0.03)", border: "1px solid rgba(220,38,38,0.06)", borderRadius: 12, transition: "all 0.2s" }}
                onMouseEnter={e => { (e.currentTarget as HTMLDivElement).style.borderColor = "rgba(220,38,38,0.2)"; (e.currentTarget as HTMLDivElement).style.background = "rgba(220,38,38,0.06)"; }}
                onMouseLeave={e => { (e.currentTarget as HTMLDivElement).style.borderColor = "rgba(220,38,38,0.06)"; (e.currentTarget as HTMLDivElement).style.background = "rgba(220,38,38,0.03)"; }}
              >
                <div style={{ width: 34, height: 34, borderRadius: 9, background: "rgba(220,38,38,0.08)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, marginTop: 2 }}>
                  <div style={{ width: 8, height: 8, borderRadius: 99, background: RED, boxShadow: `0 0 8px ${RED}88` }} />
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 3, flexWrap: "wrap" }}>
                    <span style={{ fontSize: "0.55rem", fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", color: RED, fontFamily: "Oswald, sans-serif" }}>{newsTypeLabel(item.type)}</span>
                    <span style={{ fontSize: "0.5rem", color: "#333" }}>·</span>
                    <span style={{ fontSize: "0.6rem", color: "#444", fontWeight: 600 }}>{timeAgo(item.ts)}</span>
                    {item.tag && <span style={{ fontSize: "0.5rem", fontWeight: 800, letterSpacing: "0.08em", textTransform: "uppercase", padding: "1px 7px", borderRadius: 4, background: "rgba(220,38,38,0.12)", color: RED, border: "1px solid rgba(220,38,38,0.3)", fontFamily: "Oswald, sans-serif" }}>{item.tag}</span>}
                  </div>
          ...
          <div className="animate-fade-up-delay-4 arena-card" style={{ padding: 24 }}>
            ...
            <ResponsiveContainer width="100%" height={270}>
              <RadarChart data={data.radar} margin={{ top: 10, right: 30, bottom: 10, left: 30 }}>
                <PolarGrid stroke="rgba(220,38,38,0.08)" />
                <PolarAngleAxis dataKey="subject" tick={{ fill: "#666", fontSize: 11, fontWeight: 600 }} />
                <Radar dataKey="A" stroke={RED} strokeWidth={2} fill={RED} fillOpacity={0.12} style={{ filter: "drop-shadow(0 0 10px rgba(220,38,38,0.5))" }} />
              </RadarChart>
            </ResponsiveContainer>
          </div>
            ...
            <ResponsiveContainer width="100%" height={270}>
              <PieChart>
                <Pie data={winLoss} cx="50%" cy="50%" innerRadius={65} outerRadius={105} paddingAngle={4} dataKey="value" strokeWidth={0}>
                  <Cell fill={RED} style={{ filter: "drop-shadow(0 0 12px rgba(220,38,38,0.5))" }} /><Cell fill="rgba(255,255,255,0.05)" />
                </Pie>
        ...
        <div className="animate-fade-up-delay-6 arena-card" style={{ padding: 24 }}>
          ...
          <ResponsiveContainer width="100%" height={230}>
            <AreaChart data={seasonChart} margin={{ top: 4, right: 8, left: -10, bottom: 0 }}>
              <defs>
                <linearGradient id="rG" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor={RED} stopOpacity={0.3} /><stop offset="100%" stopColor={RED} stopOpacity={0} /></linearGradient>
              ...
              <YAxis tick={{ fill: "#444", fontSize: 11 }} axisLine={false} tickLine={false} />
              <Tooltip content={<ChartTooltip />} />
              <Area type="monotone" dataKey="rating" stroke={RED} strokeWidth={2.5} fill="url(#rG)" name="Passer Rating" dot={{ fill: RED, strokeWidth: 0, r: 3.5 }} />
        ...
        <div className="animate-fade-up-delay-7 arena-card" style={{ padding: 24 }}>
          ...
          <div style={{ display: "flex", gap: 14, overflowX: "auto", paddingBottom: 8 }}>
            {data.awards.map((a, i) => (
              <div key={i} className="arena-card-neon-edge" style={{ minWidth: 160, background: "linear-gradient(135deg, rgba(220,38,38,0.06), rgba(12,12,12,0.9))", border: "1px solid rgba(220,38,38,0.15)", borderRadius: 14, padding: "18px 16px", display: "flex", flexDirection: "column", gap: 7, flexShrink: 0, transition: "transform 0.2s, box-shadow 0.2s" }}
                onMouseEnter={e => { (e.currentTarget as HTMLDivElement).style.transform = "translateY(-4px)"; (e.currentTarget as HTMLDivElement).style.boxShadow = "0 14px 40px rgba(220,38,38,0.12)"; }}
                onMouseLeave={e => { (e.currentTarget as HTMLDivElement).style.transform = ""; (e.currentTarget as HTMLDivElement).style.boxShadow = ""; }}
              >
                <span style={{ fontSize: "1.9rem" }}>{a.icon}</span>
                <span style={{ fontSize: "0.6rem", color: RED, fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", fontFamily: "Oswald, sans-serif" }}>SEASON {a.year}</span>
                <span style={{ fontSize: "0.85rem", fontWeight: 700, color: WHITE, lineHeight: 1.3 }}>{a.award}</span>
                <div style={{ height: 2.5, background: `linear-gradient(90deg, ${RED}, ${RED_B})`, borderRadius: 99, marginTop: 4, boxShadow: `0 0 8px ${RED}44` }} />
              </div>
            ))}
          </div>
        </div>
